...
class QPUClass():
    ...
    def create_graph(self):
        G = nx.Graph()
        # Add nodes
        nodes = [i for i in range(6)]  # We have 16 nodes
    ...
    def create_DQC_graph(self):
        ...
        G.add_edge(0, 6, weight=0, label="quantum", tele_qubit=False)
        G.edges[(0,6)]['mask_generate'] = True
        numNodes = len(G.nodes)
        numEdges = len(G.edges)

        return G

    def draw_DQC_graph(self):
        ...
        nx.draw(self.G, pos_G, with_labels=True, ax=ax, edge_color=colors, node_color='lightblue', node_size=300)
        #plt.show()
        self.pos = pos_G
