...
config.environment = EnvUpdater(completion_deadline = 1500 - 1, learn_from_file=True)  #1500  # how many steps we allow for the DAG to be executed
...
if __name__ == "__main__":
    ...
    for iteration in range(config.environment.quantumEnv.iteration_amount):
        ...
        while not agent.done:
            ...
            agent.conduct_action(action)  
            agent.state = agent.next_state #this is to set the state for the next iteration
            agent.mask = agent.next_mask
