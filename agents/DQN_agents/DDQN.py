...
class DDQN(DQN_With_Fixed_Q_Targets):
    ...
    def compute_q_values_for_next_states(self, next_states, next_mask):
        ...
        max_action_indexes = self.q_network_local(next_states, next_mask).detach().argmax(1)
        Q_targets_next = self.q_network_target(next_states, next_mask).gather(1, max_action_indexes.unsqueeze(1))
